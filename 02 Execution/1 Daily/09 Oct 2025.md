...
function pickTodaysTask(p) {
  const ts = p.file.tasks ?? [];
  const rx = new RegExp(`(?:\\s|\\^)${today}\\s*$`);
  const todays = ts.filter(t => rx.test(t.text));
  if (!todays.length) return null;
  return todays.find(t => !t.completed) ?? todays[0];
}

const tasks = pages.array().map(pickTodaysTask).filter(t => t);
if (tasks.length) dv.taskList(tasks, false);
else dv.paragraph("No habit tasks for today.");
```
[[🧠Mind Map]]
### Overdue Tasks
