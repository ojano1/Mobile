...
// Today's date formatted as "Thu, 9 Oct 2025"
const todayPretty = window.moment().format("ddd, D MMM YYYY");

// Find all habit files
const pages = dv.pages(`"${ROOT}"`)
...
// remove any literal "Done ..." token that might be in the text itself
const stripDoneTokens = s => s
  .replace(/\s+Done\s+\d{1,2}\s\w{3}\s\d{4}/i, "")
  .replace(/\s+done\(\d{4}-\d{2}-\d{2}\)/i, "")
  .replace(/\s+done:\s*\d{4}-\d{2}-\d{2}/i, "");

const hasToday = s => s.includes(todayPretty);

const out = [];
for (const p of pages) {
  const t = (p.file.tasks ?? []).find(x => !x.completed && hasToday(x.text));
  if (!t) continue;

  // clone & sanitize to suppress Dataview "Done/Due/Scheduled/Start" chips
  const clean = { ...t };
  clean.text = stripDoneTokens(stripLinks(t.text));

  // Remove DV-parsed date metadata so no chips render
  clean.created = null;
  clean.completion = null;
  clean.due = null;
  clean.scheduled = null;
  clean.start = null;

  // some DV versions read from 'annot' – clear it too if present
  if (clean.annot) clean.annot = null;

  out.push(clean);
}

if (out.length) {
  dv.taskList(out, false);   // live checkboxes, no metadata chips
} else {
